...
export const CategoriesPagination = ({ categoriesCount = 0, page, setPage }) => {

    const totalPages = Math.ceil(categoriesCount / 20)
    ...
    return (
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="text-sm text-slate-500 text-center sm:text-left">
                Showing <span className="font-medium text-slate-600">{page+1}</span> to <span className="font-medium text-slate-600">{totalPages}</span> of <span className="font-medium text-slate-600">{categoriesCount}</span> results
            </div>
        </div>
    );
}